...
void clear_buffer(){
    /* 
        cin.ignore(count, delim): count é o numero maximo de caracters descartados e delim é o ultimo caracter a ser descartado, quando o comando chegar nele, para.
            - ignore ate count caracters ou até encontrar delim
        std::numeric_limits<int>::max(): fornece informação sobre determinado tipo numerico (no caso int), no caso o valor maximo do int
        std::numeric_limits<std::streamsize>::max(): fornece o valor maximo de stremsize (9223372036854775807), praticamente infinito. Garante que vai apagar o buffer inteiro
    */
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    std::cout << "Type your name and age separated by a space or enter: ";
        ...
        //std::cin >> age1;
    std::cin >> name >> age1;
    std::cout << "Welcome " << name << " you're " << age1 << " years" << std::endl;
    std::cout << "--------------------------------" << std::endl;
    ...
    std::cin >> age2;
    std::cout << "Welcome " << full_name << " you're " << age2 << " years" << std::endl;
    std::cout << "--------------------------------" << std::endl;
